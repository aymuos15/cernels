...
__global__ void rope_kernel(const __half* __restrict__ q, const __half* __restrict__ k,
                            const __half* __restrict__ cs, const __half* __restrict__ sn,
...
std::tuple<at::Tensor, at::Tensor> rope(at::Tensor q, at::Tensor k, at::Tensor cos, at::Tensor sin) {
    ...
    rope_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
        ...
        reinterpret_cast<__half*>(qo.data_ptr<at::Half>()),
        reinterpret_cast<__half*>(ko.data_ptr<at::Half>()),
        n_pairs, d, s, nh);
    return {qo, ko};
}
