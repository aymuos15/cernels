"""moe loader — kernel-builder kernel under src/kops/moe/ (built with nix; see scripts/build_kernels.sh)."""
...
_mod: Any = None
_REPO = Path(__file__).resolve().parents[1] / "moe"
...
def kernel(x, moe):
    ...
    return _module().moe_grouped_gemm(x.contiguous(), w1, w2, indices, wts, topk, act_id)
