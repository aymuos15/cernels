// Fused grouped-GEMM MoE kernel for Cohere2Moe routed experts (plain SwiGLU, no bias,
// no shared expert). Port of qwen3-next-moe-experts retargeted at North Mini Code:
// E=128 experts, top-8, H=2048, F=768, sigmoid router weights (computed outside).
//
// Layout (routing precomputed on the Python side, matching transformers
// Cohere2MoeExperts; F.linear(x, W) semantics so weights are stored [out, in]):
...
// token that overhead dominates the ~75 MB of expert-weight traffic. Here the whole
...
at::Tensor cohere2_moe_experts_decode(at::Tensor x,            // [T, H]      bf16
...
at::Tensor cohere2_moe_experts(at::Tensor x,            // [T, H]      bf16
                               at::Tensor gate_up_proj, // [E, 2F, H]  bf16
                               at::Tensor down_proj,    // [E, H, F]   bf16
                               at::Tensor indices,      // [T, topk]   int32
                               at::Tensor weights,      // [T, topk]   fp32
