...
// expert_counts: [E] zero-initialized by caller.
...
// Activation in-place. act_id: 0=gelu_tanh, 1=silu, 2=relu
...
// Routing is precomputed on the Python side (matching megablocks dtype).
at::Tensor megablocks_moe(at::Tensor x,       // [T, H]      bf16
    ...
    // Prefix sum -> expert offsets (E+1 elements), on device.
    ...
    // Read expert offsets to host (E+1 ints = 36 bytes, tiny sync).
    ...
    // Per-expert cuBLAS GEMMs (no per-expert allocations).
    for (int e = 0; e < E; e++) {
