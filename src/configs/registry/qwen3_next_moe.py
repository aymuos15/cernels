...
from kops.registry.qwen3_next_moe import kernel as qwen3_next_moe_kernel


class Qwen3NextMoE(Config):
    name = "qwen3_next_moe"
    ...
    # only reaches ~1.1x over eager (as measured on gpt_oss_moe) — kept on as an honest,
    # labeled baseline rather than hidden; custom should still beat it.
    use_compile = True
    ...
    def baseline(self, x, block):  # the real transformers Qwen3NextSparseMoeBlock -> op_eager
        if block is None:
